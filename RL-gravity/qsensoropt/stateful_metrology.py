"""Module containing the :py:obj:`~.StatefulMetrology`
class
"""
...
from tensorflow import zeros, expand_dims, constant, concat, \
    reshape, sqrt, broadcast_to, eye, cast, einsum, Tensor
...
class StatefulMetrology(StatefulSimulation):
    r"""Simulation class for the standard
    Bayesian metrological task, with a stateful
    probe.
    ...
    implemented in :py:meth:`~.StatefulMetrology.loss_function`
    ...
    :py:meth:`~.StatefulMetrology.generate_input`,
    ...
    the measurement step, and the mean of the probe
    state on the posterior distribution. All this
    ...
    **Achtung!** The type of the probe state
    defined in the `state_specifics` attribute of the
    ...
    input_size: int
        ...
        method :py:meth:`~.StatefulMetrology.generate_input`.
        Given `d` the number of unknown parameters,
        the value of `input_size` is given by the
        following formula:

        ``input_size = d**2+2*d+2+state_size``

        where `state_size` is the number of scalars
        needed to represent the state of the probe.
    input_name: List[str]
    ...
    def __init__(
        ...
        of the :py:obj:`~.StatefulMetrology` class.
        ...
        cov_weight_matrix: List, optional
            ...
            by the method
            :py:meth:`~.StatefulMetrology.loss_function`.
        ...
        # Set the input size
        d = particle_filter.d
        state_filter_size = particle_filter.state_size
        input_size = d**2+2*d+2+state_filter_size
        # Set the name of each column of the input
        input_name = [f"Mean_{par.name}" for par in
                      particle_filter.phys_model.params] + \
            [f"LogDev_{par.name}" for par in
             particle_filter.phys_model.params] + \
            [f"Corr_{par1.name}_{par2.name}" for par1, par2
             in product(particle_filter.phys_model.params,
                        particle_filter.phys_model.params)] + \
            ["StepOverMaxStep", "ResOverMaxRes", ] + \
            [f"StateEntry_{i}" for i in range(state_filter_size)]
    ...
    def generate_input(
        ...
        together with the mean state of the probe
        according to the Bayesian posterior, the used
        ...
        state_ensemble: Tensor
            ...
            the state of the probe computed as if the particles of the
            ensemble were the true values of the unknown parameters.
            It is a `Tensor` of
        ...
        input_strategy: Tensor
            ...
            * The mean state of the probe, computed from
              `state_ensemble` with `weights`, using the method
              :py:meth:`~.ParticleFilter.compute_mean_state`.
              These are additional `pf.state_size` additional
              scalar inputs, which are not normalized. In
              `input_name` they are called `State_entry_{i}`, where `i`
              the index of the state entries.
        ...
        # Linear mean of the stat_filter entries
        mean_state = self.pf.compute_state_mean(
            weights,
            cast(state_ensemble, dtype=pars.prec),
        )
        mean_state = cast(mean_state, dtype=pars.prec)
        input_tensor = concat(
            [mean_tensor, log_dev_diag,
             reshape(rho, (self.bs, self.pf.d**2)),
             scaled_step, scaled_resources,
             mean_state, ], 1, name="input_tensor",
        )
        return input_tensor
    ...
    def loss_function(
        ...
        r"""Returns for each estimation in the batch
        the mean square error
        contracted with the weight matrix :math:`G`,
        i.e. teh attribute `cov_weight_matrix_tensor`
        ...
        The mean loss in :eq:`loss_G_stateful` can be expanded as
