...
class GravimeterConfig:
    """Hardware and protocol configuration for the gravimeter."""

    # Trap and NV-center constants
...
# ---------------------------------------------------------------------------
# StatelessPhysicalModel implementation
# ---------------------------------------------------------------------------
...
class GravityStatelessPhysicalModel(StatelessPhysicalModel):
    ...
    def y0_m(self, dtype) -> Tensor:
        cfg = self.cfg
        return tf.cast(
            np.sqrt(cfg.hbar_J_s / (2.0 * cfg.mass_kg * cfg.omega_rad_s)),
            dtype,
        )
    ...
    def trap_visibility_avg_small_noise(self, Bp_kTm: Tensor) -> Tensor:
        ...
        eta = self.eta(Bp_kTm)
        correction = tf.cast(
            1944.0 * (pi ** 4) * (cfg.sigma_omega_rel ** 4), eta.dtype
        ) * tf.square(eta)
    ...
    def sample_true_visibility_factor(
        ...
        if cfg.trap_visibility_mode == "none":
            ...
            delta_omega = rangen.normal(tf.shape(T_s), dtype=T_s.dtype) * tf.cast(
                cfg.sigma_omega_rel * cfg.omega_rad_s, T_s.dtype,
            )
    ...
    def _sample_mfg_rel_noise(self, shape, rangen, dtype) -> Tensor:
        bound = float(self.cfg.mfg_rel_noise_bound)
        if bound <= 0.0:
    ...
    def likelihood_given_global_params(
        ...
        g: Tensor, beta_B: Tensor, phi_off: Tensor,
    ) -> Tensor:
        T_s = controls[..., 0]
    ...
    def perform_measurement(
        ...
        theta = (
            self.k_g(T_s, Bp_kTm) * g[:, 0]
            + wrap_to_pi_tf(phi_off[:, 0] + mw_phase)
        )
