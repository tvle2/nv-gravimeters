...
def main():
    ...
    episode_len = 256 #512
    Bp_rel_bound = 0.05  # ±10% by default (paper-faithful bounded uniform noise)
    model = ModelConfig(
        ...
        sigma_dt_drift=0.0,
        Bp_rel_bound=Bp_rel_bound,
        # NEW: slow drift in eps to prevent particle impoverishment
        sigma_eps_drift=2e-4,   # start here (try 1e-4 .. 5e-4)
    )

    # ---- env config ----
    env_cfg = EnvConfig(
        episode_len=episode_len, #60
        n_g_grid=1024, #512
        n_nuisance=128, #256
        g_hist_bins=30,
        probe_every=0,
    ...
    print(f"Using bounded MFG noise: eps ~ Uniform(-{model.Bp_rel_bound:.3f}, +{model.Bp_rel_bound:.3f})")
    ...
    policy.set_action_masking(
        ...
        mu_g_index=mu_g_index,            
        mu_eps_index=mu_eps_index,        
        std_eps_index=std_eps_index,      
        ...
        wrap_max=float(env_cfg.wrap_max),             
    )
    ...
    behavior_cloning_pretrain(policy, data, device=ppo_cfg.device, epochs=6, batch_size=2048, lr=1e-3, csv_path=f"logs_{episode_len}_epsB_{format_sci_no_leading_zero(Bp_rel_bound)}/bc_metrics.csv", run_name="debug")

    # ---- [3] PPO finetune ----
    print("\n[3] PPO finetune...")
    log_path = f"logs_{episode_len}_epsB_{format_sci_no_leading_zero(Bp_rel_bound)}/train_metrics.csv"
    policy = ppo_train(
        envs=envs,
        model=policy,
        cfg=ppo_cfg,
    ...
    ckpt_path = f"checkpoints_{episode_len}_epsB_{format_sci_no_leading_zero(Bp_rel_bound)}/rbpf_policy.pt"
